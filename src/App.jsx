...
function App() {
  ...
  const handleAddToDo = (title) => {
    setTodos([
      ...todos,
      { id: uuidv4(), task: title, isEditing: false, isCompleted: false },
    ]);
  ...
  const changeStatusToEdit = (editId) => {
    setTodos(
      todos.map((todo) =>
        todo.id === editId ? { ...todo, isEditing: !todo.isEditing } : todo
      )
    );
  };
  ...
  const handleUpdateTodo = (editTitle, editId, task) => {
    setTodos(
      todos.map((todo) =>
        todo.id === editId
          ? { ...todo, task: editTitle, isEditing: !todo.isEditing }
          : todo
      )
    );
    setAlert({
      title: `Edit ${task} to ${editTitle} successfully`,
      status: 'success',
    });
  };
  ...
  const toggleCompletedTodo = (toggleId) => {
    const changeStatusToCompleted = todos.map((todo) =>
      todo.id === toggleId ? { ...todo, isCompleted: !todo.isCompleted } : todo
    );
    setTodos(changeStatusToCompleted);
    const findTask = changeStatusToCompleted.find(
      (item) => item.id === toggleId
    );
    findTask.isCompleted
      ? setAlert({ title: `Congratulation` })
      : setAlert({ title: `Keep it up` });
  };
            ...
            <span
              className={`w-64 text-sm ${
                alert.status === 'error'
                  ...
                  : 'bg-teal-700 text-white rounded-md'
              }`}
            >
              {alert.title}
            </span>
